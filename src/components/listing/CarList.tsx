...
export function CarList({ cars, onCarSelect }: CarListProps) {
    if (cars.length === 0) {
        return (
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-24">
            {cars.map((car, index) => (
                <CarListCard key={car.id} car={car} index={index} onSelect={onCarSelect} />
            ))}
        </div>
    )
}

function CarListCard({ car, index, onSelect }: { car: MapCar; index: number; onSelect?: (car: MapCar) => void }) {
    ...
    const CardContent = (
        <motion.div
            ...
            className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 h-full cursor-pointer"
        >
            {/* Image Area */}
    ...
    if (onSelect) {
        return (
            <div onClick={() => onSelect(car)} className="block group">
                {CardContent}
            </div>
        )
    }
